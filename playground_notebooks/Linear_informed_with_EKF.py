...
INPUT_FEATURES = ['synced/gyro', 'synced/acce', 'synced/magnet', 'pose/ekf_ori']
...
def preprocess_data(X, y, col_locations, test=False):
    ...
    elif 'pose/ekf_ori' in col_locations['input']:
        orientations = X[:, :, col_locations['input']['pose/ekf_ori'][0]:
                                col_locations['input']['pose/ekf_ori'][1]]
    else:
...
class Net(nn.Module):
    def __init__(self, input_size=23, hidden_size=500, output_size=3, trans_layers=5, dropout=0.001):
        super().__init__()
...
# plot predictions
ax[1].set_title('Predictions in XY-plane (test data)')
...
plt.savefig(f'../new_figures/predictions_informed_LINEAR_EKF_{timestamp_}.png')
plt.close()